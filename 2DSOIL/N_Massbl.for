...
C variables to hold mulch C and N totals
     ...
     &        CsinkHuMean, CsinkLiMean,CsinkMaMean 
        
        common /N_BAL/ModNum,CFlux,CFluxPrevious,Rflux
   ...
   !      ,'RFlux','SinkHu_C','Sinkli_C','SinkMa_C'
...
cccz why "mg"??????????????????????????????????????????????????????
       ...
       EndDo
      !  Do n=1,NumNP
      !    Rflux=Rflux+Conc(n,1)*Sink(n)*step*NO3mass_2_Nmass*nodeArea(n)
      !end do 
	 if (NShoot.eq.1)then
       Rflux=Rflux +SIncrSink  !CSink(n,1)*Step*nodeArea(n)*14./62.
       end if
        t=time
...
c  organic N is in ug/g of soil, use bd to get ug/cm3 of soil in order to sum over volume
     ...
     &             +denit(l)*Blkdn(MatNumN(l)))/3.
                CsinkHuMean=CsinkHuMean+AE*(CSinkHu(i)*
     &                    Blkdn(MatNumN(i))
     &             +CSinkHu(j)*Blkdn(MatNumN(j))
     &             +CSinkHu(l)*Blkdn(MatNumN(l)))/3.              
                CsinkLiMean=CsinkLiMean+AE*(CSinkLi(i)*
     &                    Blkdn(MatNumN(i))
     &             +CSinkLi(j)*Blkdn(MatNumN(j))
     &             +CSinkLi(l)*Blkdn(MatNumN(l)))/3.                  
                CsinkMaMean=CsinkMaMean+AE*(CSinkMa(i)*
     &                    Blkdn(MatNumN(i))
     &             +CSinkMa(j)*Blkdn(MatNumN(j))
     &             +CSinkMa(l)*Blkdn(MatNumN(l)))/3.                  
             ...
             CsinkHumus=CsinkHumus+CsinkHuMean
             CsinkLitter=CsinkLitter+CsinkLiMean
             CsinkManure=CsinkManure+CsinkMaMean
         Enddo
...
C Fact now works while there is a plant 
    ...
    !      , RFlux*fact*-1.0,         
    !        CsinkHumus*fact, CsinkLitter*fact, CsinkManure*fact
