...
! look for section with soil nitrogen information
 15       Read (40,'(A132)') InString
		...
		im = im + 1
		il = il + 1
		Do M = 1,2      ! for young and matured roots
		 Read(40, * ,ERR = 10) ConstI(m),constK(m), Cmin0(M)
           !read maximum uptake rate (mg/day/root length), concentration where uptake is half of maximum uptake, minimum concentration for uptake???
           !constK(m) MM coefficient for roots, umolNO3/cm3
           !Cmin0(M) miniumum conc NO3 at the root surface , umol NO3/cm3
		 il = il + 1
		enddo
		Read(40, * ,ERR = 10)
		im = im + 1
...
103		format(a) 
      ...
      TotalFUPM=0
       ...
       Do n=1,NumNP
          ...
          CSink(n,1)=Conc(n,1)*Sink(n)        !microg/cm3 * 1/day         
          M=MatNumN(n)
          TotalFUPY=TotalFUPY+FUP(n,2)        !FUP is root density
          TotalFUPM=TotalFUPM+FUP(n,1)
          SIncrSink2=SIncrSink2+cSink(n,1)*step*14./62.*nodeArea(n)
          !microg of Nitrogen/slab= cSink[Microgram/cm3*(1/day)]*step[day]*nodevolume[nodeareacm2*1cm]*14/62[convert from nitrate to N]
          Disp(n,1)=Dmol(1)*ThNew(n)*Tau(hNew(n))+DlngR(M)*VUP(n,1)   !VUP is water uptake per unit root length
...
C calculate nutrigen root uptake    
	 if (iSink.gt.0) Call massRootflux
       ! if (iSink.EQ.0) SIncrSink=SIncrSink2   
c -  -  -  -  -  -  -  -   -  -  -  -  -  -  -  -  -  -  -     

cccz zhuangji for node based representation  
cccz zhuangji for node based representation  
! Csink: Solute extraction rate g/cm3/day
       do n = 1, NumNP  
          
          SIncrSink=SIncrSink+CSink(n,1)*Step*nodeArea(n)*14./62.
         ! MUTIPLY BY (14/62) TO CONVERT FROM NITRATE TO NITROGEN
...
c calculate and alternative alphaK depending on demand
	   ...
	   if(FUP(n,j).gt.0) THEN
	    ...
	    RootRatio2=RootRatio*RootRatio
	     if (Disp(n,j).le.0.0) then
	      ...
	      If ((VUP(n,j).le.0).OR.(iSink.EQ.3)) Then   !diffusive uptake only
     ...
     !        /(ConstK(j)+dif)!ConstI= Imax is the constant in I max in the Michaelis Menten equation and COnstK is the Km
           !Imax= maximum uotake rate of N
           !KM= concentration where uptake is half of maximum uptake
           !Csink_m is the concentration at the root surface
           !Cmin: minimum concentration for uptake
          else
