      ...
      Include 'puweath.ins'
      Include 'puplant.ins'
	Character*10   Date4,Date1  ! date1 is dummy for beginDay until we modify the itnerface
	Character*255 RootName,T1,T2 
      Character*80 Indates,test,fle
...
C45    Continue
      ...
      read(9,'(A132)')listfle         !Files for cotton output.out
...
cdtend 8/28/98
        ...
        Do j=1,NumSD
          Conc(i,j)=0.
        Enddo
        Do j=1,NumGD  !maximum number of gases
          g(i,j)=0.
        Enddo
*
        Tmpr(i)=25.
